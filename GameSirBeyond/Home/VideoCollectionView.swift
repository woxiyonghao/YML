...
class VideoCollectionView: UICollectionView, UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout, UIScrollViewDelegate {
    ...
    var collectionViewDatas: [VideoModel] = [] {
        didSet {
    ...
    override init(frame: CGRect, collectionViewLayout layout: UICollectionViewLayout) {
        ...
        register(UINib.init(nibName: "VideoCollectionViewCell", bundle: .main), forCellWithReuseIdentifier: "VideoCollectionViewCell")
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        if indexPath.row < cellCount {
            ...
            cell.model = gameModel
            // 问题：点击cell之后，列表左右滑动，只有在VideoCollectionViewCell创建时才会触发此方法，导致无法更新所有cell的内容。无法完成需求：处于focus状态的cell播放视频，其他cell暂停视频并隐藏。解决方法：controlVideoPlaying()
