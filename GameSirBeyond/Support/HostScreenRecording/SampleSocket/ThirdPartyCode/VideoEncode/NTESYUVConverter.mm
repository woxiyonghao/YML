...
+ (NTESI420Frame *)pixelBufferToI420:(CVPixelBufferRef)pixelBuffer scale:(CGFloat)scale andOrientation:(NTESVideoPackOrientation)orientation {
    if (pixelBuffer == NULL) {
        return nil;
    }
    CVPixelBufferLockBaseAddress(pixelBuffer, 0);
...
//    ScreenRecordTime += 1;
//    CMTime frameTime = CMTimeMake(ScreenRecordTime, 600);
    CMSampleTimingInfo timing = { kCMTimeInvalid, frameTime, kCMTimeInvalid };
