...
@IBDesignable open class GSRangeSeekSlider: UIControl {
    ...
    public convenience init(frame: CGRect = .zero, completion: ((GSRangeSeekSlider) -> Void)? = nil) {
    ...
    open weak var delegate: GSRangeSeekSliderDelegate?
    ...
    @IBInspectable open var hideRightHandle: Bool = false {
        didSet {
            rightHandle.isHidden = hideRightHandle
        }
    }
    ...
    @IBInspectable open var colorSlider: UIColor?
    ...
    @IBInspectable open var sliderLineBetweenHandlesImage: UIImage? {
        didSet {
            ...
            sliderLineBetweenHandles.frame = handleFrame
    ...
    private func updateColors() {
        ...
        if let initialColor = initialColor?.cgColor, isInitial {
            ...
            let color: CGColor = (leftHandleImage == nil) ? initialColor : UIColor.clear.cgColor
            ...
            sliderLine.backgroundColor = colorSlider?.cgColor ?? tintCGColor
    ...
    fileprivate func refresh() {
        if enableStep && step > 0.0 {
            ...
            if let previousStepMinValue = previousStepMinValue, previousStepMinValue != selectedMinValue {
                GSTapticEngine.selection.feedback()
            }
        ...
        if let delegate = delegate, handleTracking != .none {
            ...
            delegate.rangeSeekSlider(self, didChange: leftHandle.frame.origin.x, maxValueFrameX: rightHandle.frame.origin.x)
        }
    }
...
private final class RangeSeekSliderLeftElement: UIAccessibilityElement {
    ...
    override func accessibilityIncrement() {
        guard let slider = accessibilityContainer as? GSRangeSeekSlider else { return }
