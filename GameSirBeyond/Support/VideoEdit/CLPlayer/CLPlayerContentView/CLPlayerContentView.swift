...
class CLPlayerContentView: UIImageView {
    ...
    private lazy var backButton: UIButton = {
        let button = UIButton()
        button.setImage(UIImage.init(named: "gamepad_key_b"), for: .normal)
        button.setTitle(i18n(string: "  Back"), for: .normal)
        button.titleLabel?.font = UIFont.systemFont(ofSize: 11, weight:  .regular)
        button.addTarget(self, action: #selector(backButtonAction), for: .touchUpInside)
        button.backgroundColor = UIColor.hex("#333634")
        
        let layout = abxyLayout()
        if layout == ABXYLayout.MFi.rawValue {
            if isConnectedLeadJoyM1Controller() {
                button.setImage(UIImage(named: "m1c_b"), for: .normal)
            }
            else {
                button.setImage(UIImage(named: "gamepad_key_b"), for: .normal)
            }
        }
        else if layout == ABXYLayout.Switch.rawValue {
            if isConnectedLeadJoyM1Controller() {
                button.setImage(UIImage(named: "m1c_a"), for: .normal)
            }
            else {
                button.setImage(UIImage(named: "gamepad_key_a"), for: .normal)
            }
        }
...
//        view.imageView?.contentMode = .scaleAspectFit;
    ...
    private lazy var highLightButton: UIButton = {
        let view = UIButton()
        view.setImage(UIImage.init(named: "ve_highLight"), for: .normal)
        view.imageView?.contentMode = .scaleAspectFit;
        view.addTarget(self, action: #selector(highLightButtonAction(_:)), for: .touchUpInside)
        return view
    }()
    ...
    private lazy var editButton: UIButton = {
        let view = UIButton()
        view.setImage(UIImage.init(named: "ve_edit"), for: .normal)
        view.imageView?.contentMode = .scaleAspectFit;
        view.addTarget(self, action: #selector(editButtonAction(_:)), for: .touchUpInside)
        return view
    }()
    ...
    private lazy var downloadButton: UIButton = {
        let view = UIButton()
        view.setImage(UIImage.init(named: "ve_download"), for: .normal)
        view.imageView?.contentMode = .scaleAspectFit;
        view.addTarget(self, action: #selector(downloadButtonAction(_:)), for: .touchUpInside)
        return view
    }()
    ...
    private lazy var deleteButton: UIButton = {
        let view = UIButton()
        view.setImage(UIImage.init(named: "ve_delete"), for: .normal)
        view.imageView?.contentMode = .scaleAspectFit;
        view.addTarget(self, action: #selector(deleteButtonAction(_:)), for: .touchUpInside)
        return view
    }()
    ...
    public func highlightPlayMode(flag:Bool) {
         self.editButton.isHidden = flag
         self.highLightButton.isHidden  = flag
...
//        bottomContentView.addSubview(fullButton)  暂时不需要
        
       //自定义增加
        bottomContentView.addSubview(highLightButton)
        bottomContentView.addSubview(editButton)
        bottomContentView.addSubview(downloadButton)
        bottomContentView.addSubview(deleteButton)
        //
    ...
    func makeConstraints() {
        ...
        bottomContentView.snp.makeConstraints { make in
            ...
            make.height.equalTo(140)   //下层操作区域高度
        ...
        backButton.snp.makeConstraints { make in
            make.left.equalTo(topToolView.snp.left).offset(40)
//            make.top.equalTo(topToolView.snp.top).offset(-40)
...
//            make.left.equalTo(backButton.snp.right).offset(15)
        ...
        playButton.positionStyle(.top,at:10)
        
       
        
        
        progressView.snp.makeConstraints { make in
            make.left.equalTo(bottomContentView.snp.left).offset(130)
//            make.centerY.equalToSuperview()
            make.top.equalTo(bottomContentView.snp.top).offset(13)
            make.height.equalTo(4)
            make.right.equalTo(bottomContentView.snp.right).offset(-130)
        }
        currentDurationLabel.snp.makeConstraints { make in
            make.right.equalTo(progressView.snp.left).offset(-15)
//            make.centerY.equalToSuperview()
            make.centerY.equalTo(progressView.snp.centerY).offset(0)
        }
        totalDurationLabel.snp.makeConstraints { make in
            make.left.equalTo(progressView.snp.right).offset(15)
//            make.centerY.equalToSuperview()
            make.centerY.equalTo(progressView.snp.centerY).offset(0)
        }
        ...
        highLightButton.snp.makeConstraints { make in
            ...
            make.height.equalTo(80)
            make.centerY.equalToSuperview().offset(15)
            make.left.equalTo(currentDurationLabel.snp.left).offset(0)
        }
        highLightButton.positionStyle(.top,at:10)
        
        editButton.snp.makeConstraints { make in
            make.width.equalTo(95)
            make.height.equalTo(80)
            make.centerY.equalToSuperview().offset(15)
            make.left.equalTo(highLightButton.snp.right).offset(0)
        }
        editButton.positionStyle(.top,at:10)
...
//        }
        ...
        deleteButton.snp.makeConstraints { make in
            ...
            make.centerY.equalToSuperview().offset(15)
            make.right.equalTo(totalDurationLabel.snp.right).offset(0)
        }
        deleteButton.positionStyle(.top,at:10)
        ...
        downloadButton.positionStyle(.top,at:10)
    }
    func updateConfig() {
...
//            guard let image = config.backImage else { return UIImage.init(named: "ve_close") }
//            return image
//        }()
...
//            guard let image = config.playImage else { return UIImage.init(named: "ve_play") }
//            return image
//        }()
//        playButton.setImage(playImage, for: .normal)
        
//        let pauseImage: UIImage? = {
//            guard let image = config.pauseImage else { return UIImage.init(named: "ve_pause") }
//            return image
//        }()
    
        let btnFont: UIFont? = {
            guard let font = config.btnFont else { return UIFont.systemFont(ofSize: 12, weight: .medium) }
            return font
        }()
        
        let btnColor: UIColor? = {
            guard let color = config.btnColor else { return UIColor.hex("e6e6e6") }
            return color
        }()
//        playButton.setImage(pauseImage, for: .selected)
        playButton.setTitle(i18n(string: "Play"), for: .normal)
        playButton.setTitle(i18n(string:"Pause" ), for: .selected)
        playButton.titleLabel?.font = btnFont
        playButton.setTitleColor(btnColor, for: .normal)
        
        highLightButton.setTitle(i18n(string:"Highlight"), for: .normal)
        highLightButton.titleLabel?.font = btnFont
        highLightButton.setTitleColor(btnColor, for: .normal)
        highLightButton.titleLabel?.numberOfLines = 0
        
        editButton.setTitle(i18n(string:"Trim"), for: .normal)
        editButton.titleLabel?.font = btnFont
        editButton.setTitleColor(btnColor, for: .normal)
        
        downloadButton.setTitle(i18n(string:"Save"), for: .normal)
        downloadButton.titleLabel?.font = btnFont
        downloadButton.setTitleColor(btnColor, for: .normal)
        
        deleteButton.setTitle(i18n(string:"Delete"), for: .normal)
        deleteButton.titleLabel?.font = btnFont
        deleteButton.setTitleColor(btnColor, for: .normal)
...
@objc private extension CLPlayerContentView {
    ...
    func highLightButtonAction(_ button: UIButton) {
        delegate?.didClickHighlightButton(in: self)
    }
    
    func editButtonAction(_ button: UIButton) {
        delegate?.didClickEditButton(in: self)
    }
    
    func downloadButtonAction(_ button: UIButton) {
        delegate?.didClickDownloadButton(in: self)
    }
    
    func deleteButtonAction(_ button: UIButton) {
        delegate?.didClickDeleteButton(in: self)
    }
