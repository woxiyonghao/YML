...
import SnapKit
import Then
class GameShellFourView: UIView {
    ...
    lazy var subtitleL:UILabel = {
        let label = UILabel().then {
            ...
            $0.textColor = .hex("#ffffff")
            $0.text = "Backbone 支持 Xbox Remote Play、PS Remote\n Play 和 Steam Link，让您可以随时随时地畅玩游戏 。"
            $0.numberOfLines = 0
        }
        return label
    }()
